...
<!--                </svg>-->
<!--              </button>111111111111111111111111111111111111111111-->
<!--            </div>-->
...
mdlist: {},
  ...
  methods: {
    ...
    mdToHtml(md, conv) {
     ...
     if (conv["time"] in this.mdlist && conv["speeches"].length ==1) {
         console.log("kkkk","mdlist")
 return this.mdlist[conv["time"]];
}
      md = this.countAndConcat(md, "```")

      var htmlMD = marked.parse(md);
      htmlMD = htmlMD.trim();
            if (conv["loading"]==false){ this.mdlist[conv["time"]] = htmlMD}
